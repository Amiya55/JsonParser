...
public:
    ...
    void syntax_check(
        ...
        JsonTypeName containerType); // may throw exception: runtime_error

private:
    // check the key in json object
    void _check_key(
        const std::string& key); // may throw exception: runtime_error
    // check the value in json object, also it can check the element in json array
    void _check_value(
        const std::string& value); // may throw exception: runtime_error
};
